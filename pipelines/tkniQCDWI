...
for (( i=0; i<${#IMGS_RAW[@]}; i++ )); do
  ...
  XFMFCN="${XFMFCN} -t [${DIR_XFM}/${IDPFX}_mod-dwi_from-raw_to-native_xfm-affine.mat,1]"
  XFMFCN="${XFMFCN} -t ${DIR_XFM}/${IDPFX}_mod-dwi_from-raw_to-native_xfm-syn+inverse.nii.gz"
  eval ${XFMSTR}
done
...
if [[ ! -f ${MASK_FRAME} ]]; then
  ...
  antsApplyTransforms -d 3 -n Linear \
    ...
    -t identity \
    -t [${DIR_XFM}/${IDPFX}_mod-dwi_from-raw_to-native_xfm-affine.mat,1] \
    -t ${DIR_XFM}/${IDPFX}_mod-dwi_from-raw_to-native_xfm-syn+inverse.nii.gz
fi
...
antsApplyTransforms -d 3 -n GenericLabel \
  -i ${MASK_FG} -o ${DIR_SCRATCH}/${IDPFX}_mask-fg.nii.gz -r ${NATIVE_REF}
MASK_FG=${DIR_SCRATCH}/${IDPFX}_mask-fg.nii.gz

if [[ -z ${MASK_BRAIN} ]]; then MASK_BRAIN=${DIR_MASK}/${IDPFX}_mask-brain.nii.gz; fi
antsApplyTransforms -d 3 -n GenericLabel \
  -i ${MASK_BRAIN} -o ${DIR_SCRATCH}/${IDPFX}_mask-brain.nii.gz -r ${NATIVE_REF}
...
antsApplyTransforms -d 3 -n GenericLabel \
  -i ${MASK_CC} -o ${DIR_SCRATCH}/${IDPFX}_mask-cc.nii.gz -r ${NATIVE_REF}
...
for (( i=0; i<${NIMG}; i++ )); do
  ...
  for (( j=0; j<{#BVAL[@]}; j++ )); do
    ...
    3dcalc -a ${IMG}[${j}] -expr a -prefix ${DIR_SCRATCH}/DWI_B${TB}_${#TLS[@]}.nii.gz
  done

  # loop over shells for metrics
  for (( j=0; j<${#BLS[@]}; j++ )); do
    TB=${BLS[${j}]}
    3dTcat -prefix ${DIR_SCRATCH}/DWI_B${TB}.nii.gz $(ls ${DIRTMP}/DWI_B${TB}_*.nii.gz)
...
FA=${DIR_SCALAR}/${IDPFX}_scalar-fa.nii.gz
