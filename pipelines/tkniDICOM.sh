...
if [[ "${HELP}" == "true" ]]; then
  ...
  echo '------------------------------------------------------------------------'
  echo "TKNI: ${FCN_NAME}"
  echo ' DESCRIPTION: DICOM to BIDS-NIfTI Conversion & Automated QC'
  echo '------------------------------------------------------------------------'
  echo ' REQUIRED ARGUMENTS:'
  echo '  --pi <name>           PI folder name (no underscores)'
  echo '  --project <name>      Project name (preferably CamelCase)'
  echo '  --id <string>         Participant identifier prefix'
  echo ''
  echo ' INPUT & PATHING:'
  echo '  --input-dcm <path>    Full path to DICOMS (directory or .zip file)'
  echo '                        Default: sourcing from ${DIR_PROJECT}/sourcedata/'
  echo '  --dir-project <path>  Base project directory'
  echo '                        Default: /data/x/projects/${PI}/${PROJECT}'
  echo '  --dir-scratch <path>  Temporary workspace for extraction/conversion'
  echo ''
  echo ' BIDS & NAMING OPTIONS:'
  echo '  --id-field <list>     Comma-separated BIDS keys (default: sub,ses)'
  echo '  --dir-id <path>       Manual override for subject BIDS directory tree'
  echo '  --add-resolution      Append voxel resolution to NIfTI filenames'
  echo '  --show-mod <list>     Modalities to include in QC report'
  echo '                        (default: anat,dwi,func,fmap,perf)'
  echo ''
  echo ' IMAGE MANIPULATION:'
  echo '  --reorient <orient>   Reorient NIfTIs to specific code (e.g., RPI, LPI)'
  echo '  --flip <axis>         Flip image along axis (x, y, or z)'
  echo '                        (Useful for ex-vivo scans in odd orientations)'
  echo ''
  echo ' FLAGS:'
  echo '  -h | --help           Display this help message'
  echo '  -v | --verbose        Enable detailed console logging'
  echo '  -n | --no-png         Disable generation of QC images'
  echo '  --force               Force re-conversion and overwrite existing files'
  echo ''
  NO_LOG=true
  exit 0
fi
