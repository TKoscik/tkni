...
# Parse inputs -----------------------------------------------------------------
OPTS=$(getopt -o hvnrk --long pi:,project:,dir-project:,\
dir-raw:,dir-save:,id:,dir-id:,base-img:,base-mod:,\
align-manual:,align-to:,fg-clip:,\
dir-scratch:,requires:,\
help,verbose,force,no-png,no-rmd,no-keep -n 'parse-options' -- "$@")
if [[ $? != 0 ]]; then
...
if [[ "${HELP}" == "true" ]]; then
  ...
  echo '------------------------------------------------------------------------'
  echo " TKNI: ${FCN_NAME}"
  echo ' DESCRIPTION: Initial anatomical processing (RPI, Denoise, N4, Alignment)'
  echo '------------------------------------------------------------------------'
  echo ' REQUIRED ARGUMENTS:'
  echo '  --pi <name>           PI folder name (no underscores)'
  echo '  --project <name>      Project name (preferably CamelCase)'
  echo '  --id <string>         Participant identifier (e.g., sub-123_ses-2024)'
  echo ''
  echo ' PATHING & DIRECTORIES:'
  echo '  --dir-project <path>  Base project directory'
  echo '                        Default: /data/x/projects/${PI}/${PROJECT}'
  echo '  --dir-id <path>       BIDS subject sub-directory (e.g., sub-123/ses-01)'
  echo '  --dir-save <path>     Directory to save derivatives (default: derivatives/tkni)'
  echo '  --dir-raw <path>      Location of raw input data'
  echo '  --dir-scratch <path>  Temporary workspace (deleted on successful exit)'
  echo ''
  echo ' PROCESSING OPTIONS:'
  echo '  --base-img <file>     Specific image to process (overrides BIDS search)'
  echo '  --base-mod <string>   Modality label (default: T1w)'
  echo '  --align-to <file>     Target template for rigid alignment'
  echo '  --align-manual <mat>  Path to a manual .mat transform if auto-ACPC fails'
  echo '  --fg-clip <float>     AFNI automask clip fraction (default: 0.5)'
  echo '  --requires <list>     Comma-separated prerequisite workflows (default: tkniDICOM)'
  echo ''
  echo ' FLAGS:'
  echo '  -h | --help           Display this help message'
  echo '  -v | --verbose        Enable detailed console output'
  echo '  -n | --no-png         Disable generation of QC images'
  echo '  -r | --no-rmd         Disable HTML report generation & QC metrics'
  echo '  -k | --no-keep        Delete intermediate "prep" files after completion'
  echo '  --force               Overwrite existing results and re-run'
  echo ''
  NO_LOG=true
  exit 0
fi
...
cp ${DIR_SCRATCH}/${IDPFX}_prep-reorient_${BASE_MOD}.nii.gz ${IMG}
if [[ ${VERBOSE} == "true" ]]; then echo -e ">>>>> Reoriented to RPI"; fi

# Denoise image ----------------------------------------------------------------
ricianDenoise --image ${IMG} --dir-save ${DIR_SCRATCH} --keep
cp ${DIR_SCRATCH}/${IDPFX}_prep-denoise_${BASE_MOD}.nii.gz ${IMG}
...
mv ${DIR_SCRATCH}/${IDPFX}_mask-fg+AUTO.nii.gz ${MASK_FG}
mv ${DIR_SCRATCH}/${IDPFX}_mask-fg+AUTO.png ${DIR_SCRATCH}/${IDPFX}_mask-fg.png
...
cp ${DIR_SCRATCH}/${IDPFX}_prep-biasN4_${BASE_MOD}.nii.gz ${IMG}
...
if [[ -z ${ALIGN_MANUAL} ]]; then
  ...
  antsApplyTransforms --d 3 -n GenericLabel \
    ...
    -t identity -t ${DIR_SCRATCH}/${IDPFX}_mod-${BASE_MOD}_from-raw_to-ACPC_xfm-rigid.mat
else
...
cp ${DIR_SCRATCH}/${IDPFX}_reg-align+ACPC_${BASE_MOD}.nii.gz ${IMG}
if [[ ${VERBOSE} == "true" ]]; then echo -e ">>>>> Rigid alignment"; fi

# Rescale intensity ------------------------------------------------------------
rescaleIntensity --image ${IMG} --mask ${MASK_FG} --dir-save ${DIR_SCRATCH}
cp ${DIR_SCRATCH}/${IDPFX}_prep-rescale_${BASE_MOD}.nii.gz ${IMG}
...
mv ${DIR_SCRATCH}/${IDPFX}_mask-brain+SYNTH.nii.gz ${MASK_BRAIN}
mv ${DIR_SCRATCH}/${IDPFX}_mask-brain+SYNTH.png ${DIR_SCRATCH}/${IDPFX}_mask-brain.png
if [[ ${VERBOSE} == "true" ]]; then echo -e ">>>>> Synthstrip brain extraction"; fi

# Make native PNG --------------------------------------------------------------
if [[ ${NO_PNG} == "false" ]]; then
  make3Dpng --bg ${IMG}
  make3Dpng --bg ${IMG} --bg-threshold "2.5,97.5" --layout "9:x;9:x;9:x" \
    --filename ${IDPFX}_slice-sagittal_${BASE_MOD}
  make3Dpng --bg ${IMG} --bg-threshold "2.5,97.5" --layout "9:y;9:y;9:y" \
    --filename ${IDPFX}_slice-coronal_${BASE_MOD}
  make3Dpng --bg ${IMG} --bg-threshold "2.5,97.5" --layout "9:z;9:z;9:z" \
...
if [[ ${NO_RMD} == "false" ]]; then
  ...
  IMG_REORIENT=${DIR_SCRATCH}/${IDPFX}_prep-reorient_${BASE_MOD}.nii.gz
  IMG_NOPROC=${DIR_SCRATCH}/${IDPFX}_prep-raw_${BASE_MOD}.nii.gz
  MASK_FRAME=${DIR_SCRATCH}/${IDPFX}_mod-${BASE_MOD}_mask-frame.nii.gz
  niimath ${IMG_REORIENT} -add 1 -bin ${MASK_FRAME}
  TXFM=${DIR_SCRATCH}/${IDPFX}_mod-T1w_from-raw_to-ACPC_xfm-rigid.mat
  if [[ -n ${ALIGN_MANUAL} ]]; then TXFM=${ALIGN_MANUAL}; fi
  antsApplyTransforms -d 3 -n BSpline[3] \
    -i ${IMG_REORIENT} -o ${IMG_NOPROC} -r ${IMG} \
    -t identity -t ${TXFM}
  antsApplyTransforms -d 3 -n GenericLabel \
    -i ${MASK_FRAME} -o ${MASK_FRAME} -r ${IMG} \
    -t identity -t ${TXFM}
  EFC_RAW=($(qc_efc --image ${IMG_NOPROC} --frame ${MASK_FRAME}))
  EFC_NTV=($(qc_efc --image ${IMG} --frame ${MASK_FRAME}))
  FBER_RAW=($(qc_fber --image ${IMG_NOPROC} --mask ${MASK_FG}))
  FBER_NTV=($(qc_fber --image ${IMG} --mask ${MASK_FG}))
  ...
  SNR_NTV_FR=($(qc_snr --image ${IMG} --mask ${MASK_FRAME}))
  SNR_NTV_FG=($(qc_snr --image ${IMG} --mask ${MASK_FG}))
  SNR_NTV_BR=($(qc_snr --image ${IMG} --mask ${MASK_BRAIN}))
  SNRD_RAW=($(qc_snrd --image ${IMG_NOPROC} --fg ${MASK_FG}))
  SNRD_NTV=($(qc_snrd --image ${IMG} --fg ${MASK_FG}))
  D_RAW_FG=($(Rscript ${TKNIPATH}/R/qc_descriptives.R -i ${IMG_NOPROC} -m ${MASK_FG} -s ${DIR_SCRATCH}/tmp))
  D_RAW_BR=($(Rscript ${TKNIPATH}/R/qc_descriptives.R -i ${IMG_NOPROC} -m ${MASK_BRAIN} -s ${DIR_SCRATCH}/tmp))
  D_NTV_FG=($(Rscript ${TKNIPATH}/R/qc_descriptives.R -i ${IMG} -m ${MASK_FG} -s ${DIR_SCRATCH}/tmp))
  D_NTV_BR=($(Rscript ${TKNIPATH}/R/qc_descriptives.R -i ${IMG} -m ${MASK_BRAIN} -s ${DIR_SCRATCH}/tmp))
...
if [[ "${NO_RMD}" == "false" ]]; then
  ...
  echo '7. Brain Extraction using Synthstrip ' >> ${RMD}
  ...
  echo '[11] Hoopes A, Mora JS, Dalca AV, Fischl B, Hoffmann M. SynthStrip: skull-stripping for any brain image. Neuroimage. 2022;260: 119474. doi:10.1016/j.neuroimage.2022.119474  ' >> ${RMD}
  echo '[12] Yushkevich PA, Piven J, Hazlett HC, Smith RG, Ho S, Gee JC, et al. User-guided 3D active contour segmentation of anatomical structures: significantly improved efficiency and reliability. Neuroimage. 2006;31: 1116–1128. doi:10.1016/j.neuroimage.2006.01.015  ' >> ${RMD}
  echo '[13] Mastering digital image alchemy. In: ImageMagick [Internet]. [cited 14 Feb 2025]. Available: https://imagemagick.org  ' >> ${RMD}
  echo '[14] Rorden C, Webster M, Drake C, Jenkinson M, Clayden JD, Li N, et al. Niimath and fslmaths: Replication as a method to enhance popular neuroimaging tools. Apert Neuro. 2024;4. doi:10.52294/001c.94384  ' >> ${RMD}
  echo '' >> ${RMD}

  echo '### Cleaned' >> ${RMD}
    TPNG=${DIR_SCRATCH}/${IDPFX}_${BASE_MOD}.png
    if [[ ! -f "${TPNG}" ]]; then make3Dpng --bg ${IMG}; fi
    echo '![Clean Native]('${TPNG}')' >> ${RMD}
    echo '' >> ${RMD}
  echo '### Raw' >> ${RMD}
    BNAME=$(basename ${IMG_RAW})
    FNAME=${IMG_RAW//\.nii\.gz}
    if [[ ! -f "${FNAME}.png" ]]; then make3Dpng --bg ${IMG_RAW}; fi
    echo '![Raw]('${FNAME}'.png)' >> ${RMD}
    echo '' >> ${RMD}
  echo '### Check Brain Extraction Quality' >> ${RMD}
    TNII=${DIR_SCRATCH}/${IDPFX}_mask-brain.nii.gz
    TPNG=${DIR_SCRATCH}/${IDPFX}_mask-brain.png
  ...
  echo '#### Click to View -->' >> ${RMD}
  echo '#### Axial' >> ${RMD}
    echo '!'"[native clean axial](${DIR_SCRATCH}/${IDPFX}_slice-axial_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}
  echo '#### Coronal' >> ${RMD}
    echo '!'"[native clean coronal](${DIR_SCRATCH}/${IDPFX}_slice-coronal_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}
  echo '#### Sagittal' >> ${RMD}
    echo '!'"[native clean sagittal](${DIR_SCRATCH}/${IDPFX}_slice-sagittal_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}

  echo '### Processing Steps {.tabset}' >> ${RMD}
  echo '#### Click to View ->' >> ${RMD}
  echo '#### RPI Reorientation' >> ${RMD}
    echo '!'"[RPI Reoriented](${DIR_SCRATCH}/${IDPFX}_prep-reorient_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}
  echo '#### Denoising' >> ${RMD}
    echo '!'"[Noise](${DIR_SCRATCH}/${IDPFX}_prep-noise_${BASE_MOD}.png)" >> ${RMD}
    echo '!'"[Denoised](${DIR_SCRATCH}/${IDPFX}_prep-denoise_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}
  echo '#### Foreground Mask' >> ${RMD}
    echo '!'"[FG Mask](${DIR_SCRATCH}/${IDPFX}_mask-fg.png)" >> ${RMD}
    echo '' >> ${RMD}
  echo '#### INU Correction' >> ${RMD}
    echo '!'"[Bias Field](${DIR_SCRATCH}/${IDPFX}_mod-${BASE_MOD}_prep-biasN4_biasField.png)" >> ${RMD}
    echo '!'"[INU Corrected](${DIR_SCRATCH}/${IDPFX}_prep-biasN4_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}
  echo '#### Rigid Alignment (ACPC-ish)' >> ${RMD}
    echo '!'"[Alignment](${DIR_SCRATCH}/${IDPFX}_reg-align+ACPC.png)" >> ${RMD}
    echo '!'"[Aligned](${DIR_SCRATCH}/${IDPFX}_reg-align+ACPC_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}
  echo '#### Intensity Rescale' >> ${RMD}
    echo '!'"[Rescaled](${DIR_SCRATCH}/${IDPFX}_prep-rescale_${BASE_MOD}.png)" >> ${RMD}
    echo '' >> ${RMD}
...
if [[ ${NO_KEEP} == "false" ]]; then
  TDIR=${DIR_SAVE}/anat/prep/${IDDIR}
  mkdir -p ${TDIR}
  cp ${DIR_SCRATCH}/${IDPFX}_prep-reorient_${BASE_MOD}.* ${TDIR}/
  cp ${DIR_SCRATCH}/${IDPFX}_prep-denoise_${BASE_MOD}.* ${TDIR}/
  cp ${DIR_SCRATCH}/${IDPFX}_prep-noise_${BASE_MOD}.* ${TDIR}/
  cp ${DIR_SCRATCH}/${IDPFX}_prep-biasN4_${BASE_MOD}.* ${TDIR}/
  cp ${DIR_SCRATCH}/${IDPFX}_mod-${BASE_MOD}_prep-biasN4_biasField.* ${TDIR}/
  cp ${DIR_SCRATCH}/${IDPFX}_prep-rescale_${BASE_MOD}.* ${TDIR}/
  cp ${DIR_SCRATCH}/${IDPFX}_reg-align* ${TDIR}/
fi
