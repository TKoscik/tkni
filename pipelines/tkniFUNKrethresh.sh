...
id:,dir-id:,\
ts:,\
do-denoise,bex-mode:,no-cat-runs,keep-runs,no-save-clean,\
anat:,anat-mask:,anat-seg:,val-csf:,val-wm:,seg-erosion:,\
compcorr-n:,bandpass-hi:,bandpass-lo:,space-coreg:,spike-thresh:,\
save-clean:,no-norm,no-censor,norm-ref:,norm-xfm:,space_norm:,\
dir-save:,dir-xfm:,dir-scratch:,\
help,verbose,loquacious,no-png,no-rmd -n 'parse-options' -- "$@")
if [[ $? != 0 ]]; then
...
# Make native space anatomical target with desired spacing
REF=${DIR_SCRATCH}/anat/anat_space-coreg.nii.gz
if [[ ${SPACE_COREG,,} == "anat" ]]; then
  cp ${DIR_SCRATCH}/raw/anat.nii.gz ${REF}
  ...
  ResampleImage 3 ${DIR_SCRATCH}/anat/anat.nii.gz ${REF} ${TSPACE} 0 0
else
  ResampleImage 3 ${DIR_SCRATCH}/anat/anat.nii.gz ${REF} ${SPACE_COREG} 0 0
fi
if [[ ${VERBOSE} == "true" ]]; then
...
for (( i=0; i<${NTS}; i++ )); do
  ...
  regressorDisplacement --regressor ${RGRS_MOCO} --dir-save ${DIR_SCRATCH}/tmp
  ...
  if [[ ${BEX_MODE,,} == "auto" ]]; then
    CLFRAC=0.5
    3dAutomask -prefix ${TS_MASK} -q -clfrac ${CLFRAC} ${TS_MEAN}
  ...
  niimath ${DIR_SCRATCH}/anat/anat.nii.gz \
    -mas ${DIR_SCRATCH}/anat/anat_mask.nii.gz \
    ${DIR_SCRATCH}/anat/anat_roi.nii.gz
  unset fcnstr
  fcnstr="coregistrationChef --recipe-name intermodalSyn"
  fcnstr="${fcnstr} --fixed ${DIR_SCRATCH}/anat/anat_roi.nii.gz"
  ...
  antsApplyTransforms -d 3 -n GenericLabel -r ${REF} \
    -i ${TS_MASK} -o ${TS_REGMASK} \
    -t identity -t ${XFM_SYN} -t ${XFM_AFFINE}
  niimath ${TS_REGMASK} ${TS_REGMASK} -odt char
  antsApplyTransforms -d 3 -n Linear -r ${REF} \
    -i ${TS_MEAN} -o ${TS_REGMEAN} \
    -t identity -t ${XFM_SYN} -t ${XFM_AFFINE}
  antsApplyTransforms -d 3 -e 3 -n Linear -r ${REF} \
    -i ${TS_MOCO} -o ${TS_REG} \
    -t identity -t ${XFM_SYN} -t ${XFM_AFFINE}
  make3Dpng --bg ${REF} --bg-thresh "2.5,97.5" \
    --fg ${TS_REGMEAN} --fg-color "hot" --fg-alpha 50 \
    --fg-mask ${TS_REGMASK} \
    --layout "6:x;6:x;6:y;6:y;6:z;6:z" --offset "0,0,0" \
  ...
  antsApplyTransforms -d 3 -n MultiLabel \
    ...
    -r ${DIR_SCRATCH}/anat/anat_space-coreg.nii.gz
  ...
  if [[ ${NO_NORM} == "false" ]]; then
    ...
    make3Dpng --bg ${DIR_SCRATCH}/xfm/norm_ref.nii.gz --bg-thresh "2.5,97.5" \
      --fg ${DIR_SCRATCH}/tmp/${TFX}_norm_proc-mean_bold.nii.gz \
      --fg-mask ${DIR_SCRATCH}/tmp/${TFX}_norm_mask-brain.nii.gz \
      --fg-color "hot" --fg-threshold "10,80" --fg-alpha 25 \
      --layout "6:x;6:x;6:y;6:y;6:z;6:z" --offset "0,0,0" \
...
if [[ ${NO_RMD} == "false" ]] || [[ ${NO_PNG} == "false" ]]; then
  ...
  for (( i=0; i<${#TMEAN[@]}; i++ )); do
    ...
    FNAME=${FNAME%%.*}
    make3Dpng --bg ${TMEAN[${i}]} --bg-threshold "2.5,97.5" \
      --fg ${TMASK[${i}]} --fg-color "#FF0000" --fg-alpha 25 --fg-cbar false \
      --layout "10:z;10:z;10:z" --offset "0,0,0" \
      --filename ${FNAME} \
