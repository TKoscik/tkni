...
if [[ "${HELP}" == "true" ]]; then
  ...
  echo '------------------------------------------------------------------------'
  echo " TKNI: ${FCN_NAME}"
  echo ' DESCRIPTION: Additional Anatomical Modality Processing & Coregistration'
  echo '------------------------------------------------------------------------'
  echo ' REQUIRED ARGUMENTS:'
  echo '  --pi <name>           PI folder name (no underscores)'
  echo '  --project <name>      Project name (CamelCase)'
  echo '  --id <string>         Participant identifier (BIDS prefix)'
  echo ''
  echo ' INPUT MODALITIES:'
  echo '  --base-mod <str>      Modality of reference image (default: T1w)'
  echo '  --add-mod <list>      Comma-separated modalities to process'
  echo '                        (default: T2w,FLAIR,SWI)'
  echo '  --base-image <file>   Override path for reference base image'
  echo '  --add-image <file>    Override path for additional image to process'
  echo '  --base-mask <file>    Brain mask for base image (auto-synth if missing)'
  echo ''
  echo ' COREGISTRATION & NORMALIZATION:'
  echo '  --coreg-recipe <str>  Recipe name (default: intermodalSyn)'
  echo '  --norm-ref <file>     Reference image for normalization'
  echo '  --norm-mat <file>     Affine transform (.mat) to reference'
  echo '  --norm-syn <file>     Warp field (Syn) to reference'
  echo '  --dir-xfm <path>      Directory containing existing transforms'
  echo ''
  echo ' OUTCOME CALCULATION:'
  echo '  --no-myelin           Skip myelin map generation (even if T1w/T2w exist)'
  echo '  --tissue <file>       Tissue segmentation mask for myelin mapping'
  echo '  --tissue-val <str>    Label values for Myelin (default: 1x2x4)'
  echo '  --no-anomaly          Skip White Matter anomaly detection for FLAIR'
  echo '  --wm-thresh <float>   SD threshold for WM anomalies (default: 2.5)'
  echo ''
  echo ' PIPELINE CONTROL FLAGS:'
  echo '  --no-reorient         Skip RPI reorientation'
  echo '  --no-denoise          Skip Rician denoising'
  echo '  --no-debias           Skip N4 bias field correction'
  echo '  --no-rescale          Skip intensity rescaling (0-10000)'
  echo '  --no-coreg            Skip coregistration to base image'
  echo '  --no-norm             Skip application of normalization transforms'
  echo '  --no-outcome          Skip modality-specific outcome calculations'
  echo ''
  echo ' GLOBAL OPTIONS:'
  echo '  -h | --help           Display this help'
  echo '  -v | --verbose        Enable verbose console logging'
