...
N=($(3dROIstats -mask ${MASK} -nzsum ${MASK}))
...
for i in "${VOLUME[@]}"; do
  ...
  niimath ${TIMG} -thr ${LO} -uthr ${HI} -binv ${TIMG} -odt char
  
  STATS=($(3dROIstats -mask ${MASK} -nzsum -prefix ${TIMG}))
  TVAL=$(echo "scale=6; ${STATS[-2]} / ${N} | bc -l)
...
if [[ ${ADD_MEAN} == "true" ]] && [[ ${NV} -gt 1 ]]; then
  VAL_M=$(printf "%.*f" ${PRECISION} ${VAL_M})
  VAL=$(echo "${VAL[@]} ${VAL_M}")
fi
echo ${VAL[@]}
